...
const QuestMapPage = () => {
  ...
  useEffect(() => {
    ...
    if (selectedRoadmap) {
      // Check if roadmap has levels, if not, initialize with sample levels
      if (!selectedRoadmap.levels || selectedRoadmap.levels.length === 0) {
        ...
        // Update the roadmap in local storage
      ...
      // Set the updated roadmap and load video links
    ...
    // Load completed levels, XP, and tasks from localStorage
    const savedCompleted = JSON.parse(localStorage.getItem(`completedLevels_${id}`)) || [];
    setCompletedLevels(savedCompleted);

    const savedXP = JSON.parse(localStorage.getItem(`xp_${id}`)) || 0;
    setXp(savedXP);

    const savedTasks = JSON.parse(localStorage.getItem(`completedTasks_${id}`)) || {};
  ...
  console.log('Roadmap Levels:', roadmap.levels); // Debugging: Check if new levels are loaded
          ...
          <ul className="space-y-3">
            {(roadmap.levels[activeLevel]?.tasks || []).map((task, i) => {
              ...
              return (
                <li
                  key={i}
                  className="flex justify-between items-center bg-gray-800 rounded-md p-3"
                >
                  <div>
